...
class Customer_rewards extends CI_Model
{
	...
	{
		$package_data_to_save = array('package_name' => $package_data['package_name'], 'deleted' => 0, 'points_percent' => $package_data['points_percent'],'rule_points'=> $package_data['rule_points']);
	...
	public function get_rule_points($package_id)
	{
		$this->db->from('customers_packages');
		$this->db->where('package_id', $package_id);

		return $this->db->get()->row()->rule_points;
	}
