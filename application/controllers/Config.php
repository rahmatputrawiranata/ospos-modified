	...
	public function save_rewards()
	{
		...
		if($customer_reward_enable)
		{
			...
			foreach($this->input->post() as $key => $value)
			{
				...
				elseif(strstr($key, 'rules_points'))
				{
					$customer_reward_id = preg_replace("/.*?_(\d+)$/", "$1", $key);
					$array_save[$customer_reward_id]['rule_points'] = $value;
				}
			...
			if(!empty($array_save))
			{
				...
				{
					// save or update
					$package_data = array('package_name' => $value['package_name'], 'points_percent' => $value['points_percent'],'rule_points' => $value['rule_points']);
